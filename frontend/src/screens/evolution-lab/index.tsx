...
import { evolutionLab, type EvolutionRequest, type SeedReason, type SeedRow } from '@/api/evolution-lab'
import { errorMessage } from '@/api/http'
import { DASH, fmt } from '@/lib/format'
import { MAX_SEEDS, useSeedPick } from '@/lib/seed-pick'
...
import { Button, Disclosure, Empty, ErrorNotice, Input, Metric, Notice, Page, PageHeader, Panel, Progress, Segmented, TEXT_TONE, signTone } from '@/ui/kit'
...
const marketKey = (region: string, delay: number, universe: string) => `${region}:${delay}:${universe}`
...
export function EvolutionLabScreen() {
  ...
  useEffect(() => {
    ...
    if (pick) set({ region: pick.scope.region, delay: pick.scope.delay, universe: pick.scope.universe, seedIds: pick.ids, autoJobId: null })
  }, [set])

  const options = useQuery({ queryKey: ['evolution-lab', 'options'], queryFn: evolutionLab.options, staleTime: 5 * 60_000 })
  ...
  // A job the backend no longer knows (it restarted) is let go; Auto Select starts another.
  useEffect(() => {
    if (job.isError) set({ autoJobId: null })
  }, [job.isError, set])
  // A finished Auto Select replaces the seeds, once.
  useEffect(() => {
    if (found && autoJobId !== appliedJobId) set({ seedIds: found.seeds.map((s) => s.alphaId), appliedJobId: autoJobId })
  ...
  const wanted = Math.min(MAX_SEEDS, Math.floor(((sized ? plan?.population : undefined) ?? 100) / 2))
  const auto = useMutation({
    mutationFn: () => evolutionLab.autoSeeds({ region: draft.region, delay: draft.delay, universe: draft.universe, count: wanted }),
    onSuccess: (r) => set({ autoJobId: r.jobId }),
    onError: (error) => toast.error(errorMessage(error)),
  })
  const busy = auto.isPending || choosing
  const selectSeeds = () => {
    useSeedPick.getState().start({ instrumentType: 'EQUITY', region: draft.region, delay: draft.delay, universe: draft.universe }, draft.seedIds)
  ...
  const markets = useMemo(() => {
    ...
    return items.some((m) => m.value === current) ? items : [{ value: current, label: `${draft.region} · D${draft.delay} · ${draft.universe}` }, ...items]
  ...
  const chooseMarket = (value: string) => {
    ...
    set({ region, delay: Number(delay), universe, seedIds: [], autoJobId: null })
  }
  ...
  const columns: Column<SeedRow>[] = [
    { key: 'alpha', header: 'Alpha', width: '96px', cell: (r) => <span className="num text-ink-muted">{r.alphaId}</span> },
    {
      key: 'expression',
      header: 'Expression',
    ...
    { key: 'turnover', header: 'Turnover', width: '88px', align: 'right', cell: (r) => fmt.pct(r.turnover) },
    {
      ...
      align: 'right',
      cell: (r) => (
        <Button size="icon-sm" variant="ghost" aria-label={`Remove ${r.alphaId}`} title="Remove" onClick={() => set({ seedIds: draft.seedIds.filter((id) => id !== r.alphaId) })}>
          <XIcon />
        </Button>
      ),
    },
  ...
  return (
    <Page>
      <PageHeader
        title="Evolution Lab"
        actions={
          <Button variant="primary" disabled={!ready} loading={add.isPending} onClick={() => simulations !== null && add.mutate(simulations)}>
            <PlusIcon />
            Add Task
          </Button>
        }
      ...
      <Panel
        ...
        actions={
          <>
            <Select label="Market" mono className="h-7 text-xs" items={markets} value={current} onChange={chooseMarket} />
            {hasSeeds && (
              <>
      ...
      >
        {choosing && (
          <div className="flex flex-col gap-1.5">
            ...
            <span className="text-xs text-ink-subtle">{job.data?.detail || 'Choosing seeds'}</span>
          </div>
        )}
        ...
        {hasSeeds ? (
          <DataTable label="Seeds" rows={plan?.seeds ?? []} columns={columns} rowKey={(r) => r.alphaId} loading={preview.isPending} maxHeight="40vh" empty="None of these Alphas can be seeds." />
        ) : (
          !choosing && (
        ...
        {found && found.seeds.length < found.wanted && (
          <Notice tone="warn" title={`${fmt.int(found.seeds.length)} of ${fmt.int(found.wanted)} seeds found in ${fmt.int(found.examined)} Alphas examined.`} />
        )}
      ...
      <Panel title="Settings">
        <div className="flex flex-col gap-4">
          ...
          <Disclosure summary="Advanced">
            <div className="flex flex-wrap items-start gap-x-8 gap-y-4">
              <Setting label="Population">
                <Segmented
                  label="Population"
                  items={[{ value: 0, label: 'Auto' }, ...(options.data?.populations ?? POPULATIONS).map((v) => ({ value: v, label: v }))]}
              ...
              <Setting label="Mutation">
                <Segmented
                  label="Mutation"
                  items={(options.data?.mutationRates ?? MUTATION_RATES).map((v) => ({ value: v, label: fmt.pct(v, 0) }))}
          ...
          <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
            ...
            <Metric boxed label="Generations" value={plan?.generations ? fmt.int(plan.generations) : DASH} />
            <Metric boxed label="Market" value={`${draft.region} · D${draft.delay}`} hint={draft.universe} />
          </div>
          <p className="text-xs text-ink-subtle">Scored on Fitness over the first 8 years; the last 2 stay untouched.</p>
                ...
                {plan.sample.map((s, i) => (
                  <li key={i} className="flex flex-wrap items-baseline justify-between gap-x-3 gap-y-0.5">
                    ...
                    <span className="text-xs text-ink-subtle">
                      {String(s.settings.neutralization ?? DASH)} · Decay {String(s.settings.decay ?? DASH)}
                    </span>
                  </li>
                ))}
              </ul>
...
function Reasons({ items }: { items: SeedReason[] }) {
  return (
    <ul className="flex max-h-64 flex-col gap-1 overflow-y-auto">
      {items.map((r) => (
        <li key={r.alphaId} className="flex gap-3 text-xs">
          <span className="num w-20 shrink-0 text-ink-muted">{r.alphaId}</span>
          <span className="text-ink-subtle">{r.reason}</span>
        </li>
      ))}
    </ul>
  )
