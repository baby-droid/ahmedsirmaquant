...
import type { TemplateDoc } from '@/lib/template-tree'
  ...
  persist(
    (set, get) => ({
      ...
      edit: (doc) => {
        ...
        set({ doc, dirty: true, past: current ? [...past, current].slice(-UNDO_STEPS) : past })
      },
      undo: () => {
        const { past } = get()
        if (past.length === 0) return
        set({ doc: past[past.length - 1], past: past.slice(0, -1), dirty: true })
      },
