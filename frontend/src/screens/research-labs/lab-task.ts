...
import { useDatasetPick, type PickFrom } from '@/lib/dataset-pick'
...
export function useLabMarket(draft: LabMarket, set: (change: Partial<LabMarket>) => void, from: PickFrom) {
  ...
  useEffect(() => {
    ...
    if (pick) set({ region: pick.scope.region, delay: pick.scope.delay, universe: pick.scope.universe, datasetIds: pick.ids })
  }, [from, set])

  const datasets = useQuery({ queryKey: ['catalog', 'datasets', scope, ''], queryFn: () => catalog.datasets(scope), enabled: chosen })
