...
from ..schemas import clip, loads_or
...
class ChatService:
    ...
    async def say(
        ...
        *,
        ...
        fields, catalog_note = await self._field_menu(scope, dataset_ids)
        if not fields:
            raise ValueError(
        ...
        available = set(await self._field_ids(scope, dataset_ids))
    ...
    async def _field_menu(
        ...
        """The fields the assistant may choose from, as compact text.
    ...
    async def _field_ids(self, scope: Tuple4, dataset_ids: list[str] | None) -> list[str]:
        from ..catalog.queries import FieldFilter
        ...
        return [str(r["field_id"]) for r in page["results"]]
...
def _transcript(history: list[ChatMessage]) -> str:
    ...
    lines = [f"  {'They' if m.role == 'user' else 'You'}: {m.text.strip()[:600]}" for m in recent]
    return "THE CONVERSATION SO FAR:\n" + "\n".join(lines)
...
def _parse(text: str) -> dict[str, Any]:
    ...
    return {
        ...
        "datasets": [str(d) for d in payload.get("datasets", []) if d],
    }
