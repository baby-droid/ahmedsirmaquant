...
over the first eight, which BRAIN reports in the ``train`` block (checked live 2026-09-11).
...
from ..plan.yields import IGNORED_CHECKS
from ..vault.mixing import DEFAULT_MAX_CORRELATION, MIN_OVERLAP
...
def request_for(tree: Node, settings: dict[str, Any], run: dict[str, Any]) -> SimulationRequest:
...
def independent(order: list[str], series: dict[str, Series | None], limit: int) -> list[str]:
...
async def _parents(optimizer: Optimizer, ranked: list[Trial], limit: int) -> list[Trial]:
    ...
    days = await optimizer.alphas.train_pnl(ids) if optimizer.alphas else {}
...
async def breed(
    optimizer: Optimizer, row: Study, trials: list[Trial], want: int
    ...
    market = await market_for(
        optimizer.alphas.catalog if optimizer.alphas else None,  # type: ignore[arg-type]
        operators,
        run["region"],
        int(run["delay"]),
        run["universe"],
        list(run.get("neutralizations") or []),
    )
    if market is None:
        return []
    population = int(run.get("population") or 100)
    scored = [
        t
        for t in trials
        if t.state == TrialState.COMPLETE and t.values and float(t.values[0]) > FAILURE
    ]
    ranked = sorted(scored, key=lambda t: float(t.values[0]), reverse=True)[:population]
    ...
    generation = 1 + sum(1 for t in trials if (t.generation or 0) > 0) // population
    seen = {ga_key(t.expression, t.settings) for t in trials}
    rate = float(run.get("mutationRate") or MUTATION_RATE)
    rng = random.Random()
