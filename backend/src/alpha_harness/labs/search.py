...
def field_choices(space: dict[str, Any]) -> dict[str, list[str]]:
    ...
    for universe in space["universes"]:
        missing = set(absent.get(universe) or ())
...
def first_pass(space: dict[str, Any], field_id: str) -> dict[str, Any]:
    ...
    absent = space.get("absent") or {}
    universe = next(
        (u for u in space["universes"] if field_id not in set(absent.get(u) or ())),
        space["universes"][0],
    )
...
def request_for(params: dict[str, Any], run: dict[str, Any]) -> SimulationRequest:
...
def draw(
    trial: Any, run: dict[str, Any], choices: dict[str, list[str]]
) -> tuple[dict[str, Any], SimulationRequest | None]:
    """One point of the search and the simulation it becomes (see ``labs.tasks``)."""
