...
def operator_table(
    ...
    a regular alpha because it shares ``group_neutralize``'s category and arity (seen live
    2026-09-10). Only the first signature counts (``bucket`` lists two, ``add`` appends
...
def validate(tree: Node, table: dict[str, OperatorInfo], known_names: set[str]) -> list[str]:
